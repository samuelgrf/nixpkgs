...
let
  release_version =
    if customVersion == null then "11.0.0"
    else customVersion.release;
  # differentiating these (variables) is important for RCs
  version =
    if customVersion == null then release_version
    else "unstable-${customVersion.version}";
  ...
  fetch =
    ...
    then
      ...
      _: _: fetchFromGitHub {
        inherit (customVersion) owner repo rev sha256;
      };

  clang-tools-extra_src = fetch "clang-tools-extra" "02bcwwn54661madhq4nxc069s7p7pj5gpqi8ww50w3anbpviilzy";

  tools = stdenv.lib.makeExtensible (tools: let
    callPackage = newScope (tools // { inherit stdenv cmake libxml2 python3 isl release_version version customVersion fetch; });
  ...
  libraries = stdenv.lib.makeExtensible (libraries: let
    callPackage = newScope (libraries // buildLlvmTools // { inherit stdenv cmake libxml2 python3 isl release_version version customVersion fetch; });
  in {
